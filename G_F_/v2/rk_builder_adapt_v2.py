...
def build_rk_green_for_bounce(bounce_npz_filename, s2, n_mode,
    ...
    print("\n===============================================")
    print("Building RK Green (adaptive v2) for bounce file:", bounce_npz_filename)
    print("  tag        =", tag)
    ...
    # ------------------------------------------------------------------
    # Green's function assembly (corrected r < r' branch)
    #
    # From the notes (page 9), the Wronskian computed by the code
    # satisfies:  omega[alpha,beta] = C^T[alpha,beta]
    # so:         omega_inv = (C^T)^{-1} = (C^{-1})^T
    #
    # The two branches of the Green's function are:
    #
    #   r >= r':  G_{ij} = sum_{a,b} f+_i(r)[a] C^{-1}_{ba} f-_j(r')[b]
    #           = f_plus(r) @ omega_inv @ f_minus(r').T
    #
    #   r <  r':  G_{ij} = sum_{a,b} f-_i(r)[a] C^{-1}_{ab} f+_j(r')[b]
    #           = f_minus(r) @ omega_inv.T @ f_plus(r').T
    #
    # The old code used the same formula for both branches, which
    # transposes the field indices (i <-> j) in the r < r' region.
    # This is the cause of the G12 test failure.
    # ------------------------------------------------------------------
    ...
    for k, r in enumerate(r_grid):
        ...
        for l, rp in enumerate(r_grid):
            ...
            f_reg_rp = f_minus[l]
            if r >= rp:
                g_rk[k, l] = f_dec_r @ omega_inv @ f_reg_rp.T
            else:
                g_rk[k, l] = f_reg_r @ omega_inv_T @ f_dec_rp.T
...
def compute_wronskian_scaled_for_bounce(bounce_npz_filename, s2, n_mode,
                                        n_eval=2000, r0=1e-4):
    data = np.load(bounce_npz_filename, allow_pickle=True)
    ...
    print("\n===============================================")
    print("Computing Wronskian (scaled) for bounce file:", bounce_npz_filename)
    print("  tag        =", tag)
    print("  false_vac  (orig) =", false_vac)
    print("  true_vac   (orig) =", true_vac)
    print("  R range    =", r_bounce[0], "→", r_bounce[-1])
    ...
    (B, dB, K_matrix, A_i, _Q_matrix, _M_free,
     _x_prime_bounce, _y_prime_bounce) = build_fluctuation_data_prime(
        pot_lin,
        r_bounce,
        x_prime,
        y_prime,
    ...
    return r_grid, w_scaled, omega, false_index, true_index


if __name__ == "__main__":
    def s2_tag(x, digits=6):
        return f"{float(x):.{digits}f}".replace(".", "p")

    parser = argparse.ArgumentParser(
        description="Build adaptive RK Green files for all bounce_data_F*_T*.npz."
    )
    parser.add_argument("--s2", type=float, default=2.0, help="Spectral parameter s2 (default: 2.0).")
    parser.add_argument("--n-mode", type=int, default=3, help="Mode number n (default: 3).")
    parser.add_argument("--n-eval", type=int, default=2000, help="Number of output r-grid points (default: 2000).")
    parser.add_argument("--r0", type=float, default=1e-4, help="Start radius r0 (default: 1e-4).")
    parser.add_argument(
        "--out-template",
        default="rk_green_data_F{false}_T{true}_n{n}_s2{s2}.npz",
        help=(
            "Output filename template. Fields: {false}, {true}, {n}, {s2}. "
            "Default includes n and s2 to avoid overwriting."
        ),
    )
    parser.add_argument(
        "--overwrite",
    ...
    for bf in bounce_files:
        ...
        iF = int(b["false_index"]) if "false_index" in b.files else -1
        iT = int(b["true_index"]) if "true_index" in b.files else -1
        out_name = args.out_template.format(
            false=iF,
            true=iT,
            n=int(args.n_mode),
            s2=s2_tag(args.s2, 6),
    ...
    print(
        f"\n[INFO] Finished building RK Greens (adaptive v2) for all bounces "
        f"at n={args.n_mode}, s2={args.s2:g}."
    )

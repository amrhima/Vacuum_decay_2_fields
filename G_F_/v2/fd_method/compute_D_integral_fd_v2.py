#!/usr/bin/env python3
"""
compute_D_integral_fd_ver2.py -- Same as compute_D_integral.py but reads
the finite-difference (v2) outputs for the n=0 and n=1 sectors:

  n=0:   gbar_n0_fd_ver2_F{F}_T{T}.npz  (from compute_gbar_n0_fd_ver2.py)
  n=1:   gbar_n1_fd_ver2_F{F}_T{T}.npz  (from compute_gbar_n1_fd_ver2.py)
  n>1:   gbar_bounce_*_npos_s2*.npz, gbar_fv_*_npos_s2*.npz  (unchanged)

The FD outputs differ from the RK outputs in two places:
  * field names:  gbar_n{0,1} -> gbar_n{0,1}_raw,
                  alpha_refined / s_star -> pole_s2 (= -lambda eigenvalue)
  * pole subtraction is done analytically by a projector inside
    gbar_sub_fd, so there is no fitted residue A_fit; the residue is
    exactly 1 for n=0 (negative mode) and exactly 4 for n=1 (4 translation
    zero modes, baked in via `degeneracy` in compute_gbar_n1_fd_ver2.py).

Output: D_integral_fd_ver2_F{F}_T{T}.npz
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute the fluctuation determinant D from G_bar data "
                    "(FD ver2 inputs for n=0 and n=1)."
    )
    ...
    # ==================================================================
    #  LOAD n=0 data (FD ver2)
    # ==================================================================
    n0_file = find_file(f"gbar_n0_fd_ver2_{tag}.npz", search_dirs)
    if n0_file is None:
        print("[ERROR] gbar_n0_fd_ver2_*.npz not found. "
              "Run compute_gbar_n0_fd_ver2.py first.")
        sys.exit(1)
    print(f"[LOAD] n=0 (fd ver2): {n0_file}")

    d0 = np.load(n0_file, allow_pickle=True)
    s2_n0       = d0["s2_grid"]
    gbar_n0     = d0["gbar_n0_raw"]
    gbar_n0_sub = d0["gbar_n0_sub"]
    # FD ver2: pole_s2 = -lambda_neg is the n=0 pole location ("alpha")
    alpha       = float(d0["pole_s2"])
    print(f"  {len(s2_n0)} points, alpha (= -lambda_neg) = {alpha:.8f}")

    # ==================================================================
    #  LOAD n=1 data (FD ver2)
    # ==================================================================
    n1_file = find_file(f"gbar_n1_fd_ver2_{tag}.npz", search_dirs)
    if n1_file is None:
        print("[ERROR] gbar_n1_fd_ver2_*.npz not found. "
              "Run compute_gbar_n1_fd_ver2.py first.")
        sys.exit(1)
    print(f"[LOAD] n=1 (fd ver2): {n1_file}")

    d1 = np.load(n1_file, allow_pickle=True)
    s2_n1       = d1["s2_grid"]
    gbar_n1     = d1["gbar_n1_raw"]
    gbar_n1_sub = d1["gbar_n1_sub"]
    # FD ver2: pole_s2 = -lambda_zm is the n=1 zero-mode pole location.
    # The residue is fixed analytically by the projector and folded into
    # the saved `degeneracy` (4 = (1+1)^2 translation modes).
    s_star      = float(d1["pole_s2"])
    A_fit       = float(d1["degeneracy"]) if "degeneracy" in d1.files else 4.0
    print(f"  {len(s2_n1)} points, s_star (= -lambda_zm) = {s_star:.8f}, "
          f"A (degeneracy) = {A_fit:.4f}")
    ...
    print("\n" + "=" * 60)
    print("D-INTEGRAL RESULTS (FD ver2)")
    print("=" * 60)
    ...
    out_name = args.out
    if out_name is None:
        out_name = os.path.join(data_dir, f"D_integral_fd_ver2_{tag}.npz")
    out_dir_check = os.path.dirname(out_name) or "."
    if not os.path.isdir(out_dir_check):
        out_name = os.path.join(script_dir, f"D_integral_fd_ver2_{tag}.npz")
    ...
    np.savez(
        ...
        n_max=args.n_max,
        bounce_tag=tag,
        # provenance: which n=0 / n=1 sources were used
        n0_source=os.path.basename(n0_file),
        n1_source=os.path.basename(n1_file),

#!/usr/bin/env python3
"""
plot_D_integral_fd_ver2.py -- Visualize the fluctuation determinant D
computed via the FD ver2 path.

Reads output of compute_D_integral_fd_ver2.py and produces:
  ...
  Figure 4: 3D surface G_bar_bounce(n, s2)
  Figure 5: 3D surface G_bar_FV(n, s2)
  Figure 6: ln|D| vs n_max
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot D-integral (FD ver2): contributions, total, "
                    "and G_bar vs n."
    )
    parser.add_argument("data", nargs="?", default=None,
                        help="Path to D_integral_fd_ver2_*.npz "
                             "(auto-detected if omitted).")
    parser.add_argument("--no-show", action="store_true")
    parser.add_argument("--save-prefix", default="",
                        help="Save figures with this prefix (e.g. 'D_plots_fd_ver2').")
        ...
        for d in [".", script_dir,
            ...
            if os.path.isdir(d):
                for f in sorted(os.listdir(d)):
                    if (f.startswith("D_integral_fd_ver2_")
                            and f.endswith(".npz")
                            and "_per_n" not in f):
                        candidates.append(os.path.join(d, f))
        if not candidates:
            print("[ERROR] No D_integral_fd_ver2_*.npz found. "
                  "Run compute_D_integral_fd_ver2.py first.")
            sys.exit(1)
    ...
    ax1a.set_title("n=0 contribution (neg mode subtracted, FD ver2)")
    ...
    ax1b.set_title("n=1 contribution (zero mode subtracted, FD ver2)")
    ...
    ax2a.set_title("D-integral components (union grid, FD ver2)")
    ...
    ax2b.set_title("Total D-integral vs s2 (FD ver2)")
    ax2b.legend(fontsize=9)
    ax2b.grid(alpha=0.3)

    fig2.suptitle(
        f"D-integral (FD ver2)  |  S_eff(no ct) = (1/2)ln|D| = {-0.5*I_total:.6f}",
    ...
    if os.path.isfile(per_n_path):
        ...
        ax3.set_title(f"G_bar bounce vs partial wave n  (n=2..{n_max}, FD ver2)")
        ...
        # Load n=0 subtracted (FD ver2) and interpolate onto npos s2 grid
        ...
        for d in search_dirs_scan:
            if not os.path.isdir(d):
                continue
            import glob as _gl
            n0_files = sorted(_gl.glob(os.path.join(d, "gbar_n0_fd_ver2_*.npz")))
            if n0_files:
        ...
        # Load n=1 subtracted (FD ver2) and interpolate onto npos s2 grid
        for d in search_dirs_scan:
            if not os.path.isdir(d):
                continue
            n1_files = sorted(_gl.glob(os.path.join(d, "gbar_n1_fd_ver2_*.npz")))
            if n1_files:
        ...
        ax4.set_title(
            f"G_bar bounce(n, s2)  surface  "
            f"(n=0..{n_max}, FD ver2; subtracted for n=0,1)"
        )
        fig4.tight_layout()
    ...
    if os.path.isfile(per_n_path):
        ...
        ax6.set_title(
            "ln|D| vs n_max  (= - total integral up to n_max, no counterterm; FD ver2)"
        )
        ax6.grid(alpha=0.3)

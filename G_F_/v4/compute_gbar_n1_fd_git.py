#!/usr/bin/env python3
"""
compute_gbar_n1_fd_git.py  --  Compute gbar(n=1, s^2) for the two-field
false-vacuum-decay fluctuation determinant via the finite-difference (FD)
sparse-LU path.
...
eigenvalue), instead of the analytic formula r^(3/2)*phi_b'(r). This makes
the projector match the discrete operator exactly, removing the residual
hairpin in G_bar^sub_n=1 right at the (discrete) zero-mode pole.

The script performs a sanity check on the returned chi_zm: it cross-checks
its overlap with the analytic r^(3/2)*phi_b' mode and warns (or, under
--strict-zm, aborts) if the overlap is low or |lambda_zm| is too large, to
catch the case of a spurious non-translational mode being picked.
...
def build_s2_grid_n1(s2_min=1e-6, s2_max=10.0,
    ...
    """
    Same grid as compute_gbar_n1.py (RK method), plus a coarse tail
    (step = tail_step) for s^2 in (tail_start, s2_max] so that
    extending s2_max from 10 to 50 does not multiply the grid size by 50.
    """
    if s2_min <= 0:
    ...
    # The "fine linear" portion runs up to min(s2_max, tail_start);
    # if s2_max > tail_start, an additional coarse tail is appended.
    fine_upper = min(s2_max, tail_start)
    parts = [np.array([s2_min, s2_max])]
    if s2_transition <= s2_min:
        parts.append(np.arange(s2_min, fine_upper + 0.5 * linear_step,
                               linear_step))
    elif s2_transition >= fine_upper:
        n_decades = np.log10(fine_upper / s2_min)
        n_log = max(int(n_decades * log_ppd), 10)
        parts.append(np.geomspace(s2_min, fine_upper, n_log))
    else:
        ...
        parts.append(np.arange(s2_transition + linear_step,
                               fine_upper + 0.5 * linear_step, linear_step))
    if s2_max > tail_start:
        parts.append(np.arange(tail_start + tail_step,
...
def main():
    ...
    # degeneracy = (1+1)**2 = 4 counts the n=1 spherical-harmonic components on S^3.
    degeneracy = (1 + 1) ** 2
    ...
    for i, s2 in enumerate(s2_grid):
        ...
        # (rank-1 identity Tr(P A^-1 P) = Tr(A^-1) - chi_zm^T A^-1 chi_zm).
        # Inside skip_radius the raw trace is NaN by design (too close to the
        # discrete zero-mode pole), so keep the gbar_sub_fd /
        # hutchinson_trace_projected fallback for the subtracted value. NaNs
        # propagate (fail-loud), never masked. The degeneracy scaling below is
        # unchanged and still applies to whatever values are returned.

...
if __name__ == "__main__":
    # Diagnostic plots are saved next to this file's project root under
    # pictures/; plt.show() is only called below because we are running as the
    # main script, so automated/headless runs that import this module never
    # block on an interactive window.
    ...
    for (iF, iT) in pairs:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(PICTURES_DIR, f"bounce_profile_prime_F{iF}_T{iT}.png"),
            dpi=150,
        )
        plt.show()
    ...
    if successful > 0:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(PICTURES_DIR, "bounce_paths_field_space_all.png"),
            dpi=150,
        )
        plt.show()

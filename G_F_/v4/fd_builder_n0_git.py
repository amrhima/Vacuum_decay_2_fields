#!/usr/bin/env python3
"""
fd_builder_n0_git.py  --  Finite-difference + Feshbach-projector builder
for the n=0 (negative-mode) sector.

Specialised counterpart of fd_builder_git.py: contains only the
machinery used by compute_gbar_n0_fd_git.py.

Here "gbar" denotes the s^2-resolved per-mode contribution to the
fluctuation-determinant sum, i.e. the trace estimate
gbar(s^2) = trace((M_tilde_n + s^2 I)^(-1)) (raw) or its negative-mode-
projected counterpart trace(P (M_tilde_n + s^2 I)^(-1) P) (subtracted).
...
def gbar_sub_fd(M_tilde, s2, dr, N2, chi, K=100, rng=None,
    ...
    compute_gbar_n0_fd_git.py excludes the immediate pole region, so the LU
...
def gbar_raw_and_sub_fd(M_tilde, s2, dr, N2, chi, K=100, rng=None,
    ...
    This replaces the second 400-probe Hutchinson pass (formerly gbar_sub_fd)
    by a single extra back-solve  chi^T A^(-1) chi.  The raw estimate reuses the
    existing hutchinson_trace_raw loop on the same LU, so the subtracted mean is
    raw_mean - chiAchi with the SAME Monte-Carlo SEM as the raw estimate.

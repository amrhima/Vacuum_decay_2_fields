"""Two-field potential definition and primed-coordinate frame.

Defines the two-field false-vacuum-decay potential V(x, y) with its five
couplings (a0, a_xy2, a_y4, a_x2y2, a_x4) symbolically via SymPy, lambdifies
its value, gradient, and Hessian to NumPy, and provides tooling to locate and
classify the critical points / vacua of the original potential.

It also supplies the ``CTShiftedLiftedPotential`` class, which shifts the
chosen false vacuum to the origin and rotates into the mass-eigenstate
("primed") coordinates that diagonalise the Hessian there, exposing the
potential, gradient, and Hessian in that frame. This module is imported by
every other module in the pipeline.

The ``CTShiftedLiftedPotential`` constructor can print rotation/eigenvalue
diagnostics; these are gated (verbose flag, default on only when this file is
run directly) so that importing it as a library stays silent.
"""
...
# [vx-opt] Batched Hessian: evaluate H_func ONCE on a whole array of points
# (broadcast over the leading axes) and stack the four entries into a
# (..., 2, 2) tensor, instead of calling H_numeric in a Python loop.  This
# is additive; the scalar H_numeric / H() above is left untouched.
def H_numeric_batch(X, params: Sequence[float] | None = None) -> np.ndarray:
    p = _unpack_params(params)
    X = np.asarray(X, dtype=float)
    x_val = X[..., 0]
    y_val = X[..., 1]
    H = H_func(x_val, y_val, *p)  # 2x2 list-of-lists, each entry a (...) array
    out = np.empty(np.shape(x_val) + (2, 2), dtype=float)
    for i in range(2):
        for j in range(2):
            out[..., i, j] = np.broadcast_to(
                np.asarray(H[i][j], dtype=float), np.shape(x_val))
    return out
...
class CTShiftedLiftedPotential:
    ...
    def __init__(self, params, false_vac, verbose=False):
        ...
        # Diagnostics are gated so that importing this module as a library is
        # silent; they only print when explicitly requested via verbose=True
        # (e.g. when this file is run directly under __main__).
        if verbose:
    ...
    # [vx-opt] Batched primed-coordinate Hessian.  Rotates a whole array of
    # original-coordinate Hessians (..., 2, 2) into primed coordinates via
    # L^T H L in a single einsum.  Additive; the scalar H() above (used by
    # the ODE RHS) is unchanged.
    def H_batch(self, Xp):
        return np.einsum(
            'ai,...ij,jb->...ab',
            self.LT,
            H_numeric_batch(self.to_original(Xp), self.params),
            self.L,
        )
...
if __name__ == "__main__":
    ...
    if len(minima) >= 1:
        ...
        pot_prime = CTShiftedLiftedPotential(p, false_vac, verbose=True)

...
def main() -> None:
    ...
    for s2 in s2_values_n1:
        tag = s2_tag(s2, 6)
        summary_name = f"gbar_bounce_F{false_index}_T{true_index}_n1_s2{tag}.npz"
        if os.path.exists(summary_name) and not args.overwrite and not args.merge:
            print(f"[SKIP] {summary_name} already exists.")
            continue
        ...
        rk_name = f"rk_green_data_n1_s2{tag}.npz"
        def rebuild_rk(n_mode=1, rk_name=rk_name, s2=s2):
            build_rk_green_for_bounce(
                bounce_path,
                s2=s2,
                n_mode=n_mode,

...
def build_and_plot_green(B, dB, K_matrix, A_i,
    ...
    for k in range(Nr):
        ...
        for l in range(Nr):
            ...
            # Basis for constant diagonal matrix diag(c0,c1)
            G_mod_basis[0, k, l] = F_big @ M00 @ F_small.T
            G_mod_basis[1, k, l] = F_big @ M11 @ F_small.T

            # Explicit G_mod_const from C_mod_inv
...
# ------------------------------------------------------------
    ...
    def test_continuum_green_equation(r_grid, G_full, H_full_arr, n_mode, tag):
        Nr = len(r_grid)
        dr = r_grid[1] - r_grid[0]
        ell_term = n_mode * (n_mode + 2)
    ...
    # choose a few representative r' columns to test
        ...
        print("\n[Continuum Green test] L_cont G_RK ≈ δ(r-r')/r'^3")
        ...
        # interior points: use the same 3-point stencil as in your FD L
            for k in range(1, Nr-1):
                ...
                # build φ_i(r) = G_{ij}(r, r')
                    G_im1 = G_full[k-1, ell, :, j_field]  # shape (2,)
                    G_i   = G_full[k,   ell, :, j_field]
                    G_ip1 = G_full[k+1, ell, :, j_field]
    ...
    # ------------------------------------------------------------
    test_continuum_green_equation(r_grid, G_full, H_full_arr, n_mode, tag)
    ...
    Cinv_best = None  # will hold best-fit diag(c0,c1) if found
...
# right after building G_mod_best:
